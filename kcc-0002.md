...
| `0x03` | `p2sh/v1`             | `byte[32]` containing `Blake2b(R)`              | One exact Kaspa Script program |
...
This is the version-0 P2SH script public key defined by KCC1. Because the
matching input's program `R` must be revealed and accept the transaction, it
participates as an authority according to its own script logic.
